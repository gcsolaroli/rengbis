...
import rengbis.Schema.{ BinaryConstraint, BoundConstraint, BoundOp, ListConstraint, NumericConstraint, TextConstraint, TimeConstraint }
...
object Validator:
    ...
    case class DeprecationWarning(message: String)

    final case class ValidationResult(value: Valid | Chunk[ValidationError], warnings: Chunk[DeprecationWarning] = Chunk.empty):
        ...
        def withWarning(warning: DeprecationWarning): ValidationResult             = copy(warnings = warnings :+ warning)
        def withWarnings(newWarnings: Chunk[DeprecationWarning]): ValidationResult = copy(warnings = warnings ++ newWarnings)

    object ValidationResult:
        def reportError(message: String): ValidationResult                  = ValidationResult(Chunk(ValidationError(message)))
        def reportWarning(message: String): ValidationResult                = ValidationResult(Valid(), Chunk(DeprecationWarning(message)))
        ...
        def summarize(values: Iterable[ValidationResult]): ValidationResult =
            ...
            val warnings: Chunk[DeprecationWarning] = Chunk.fromIterable(values.flatMap(_.warnings))
            if errors.isEmpty then ValidationResult(Valid(), warnings)
            else ValidationResult(Chunk.fromIterable(errors), warnings)
    ...
    object TextValidation:
        ...
        def validateBound(bound: BoundConstraint[Int], len: Int, label: String): ValidationResult =
            bound.op match
                case BoundOp.Exact        => if len == bound.value then ValidationResult.valid else ValidationResult.reportError(s"$label constraint (${ bound.value }) not met: ${ len }")
                case BoundOp.MinInclusive => if len >= bound.value then ValidationResult.valid else ValidationResult.reportError(s"min$label constraint (${ bound.value }) not met: ${ len }")
                case BoundOp.MinExclusive => if len > bound.value then ValidationResult.valid else ValidationResult.reportError(s"min$label constraint (> ${ bound.value }) not met: ${ len }")
                case BoundOp.MaxInclusive => if len <= bound.value then ValidationResult.valid else ValidationResult.reportError(s"max$label constraint (${ bound.value }) not met: ${ len }")
                case BoundOp.MaxExclusive => if len < bound.value then ValidationResult.valid else ValidationResult.reportError(s"max$label constraint (< ${ bound.value }) not met: ${ len }")

        def validateConstraints(constraints: TextConstraint.Constraints, text: String): ValidationResult =
            val len     = text.length
            val results = Seq(
                constraints.size.flatMap(_.min).map(bound => validateBound(bound, len, "Length")),
                constraints.size.flatMap(_.max).map(bound => validateBound(bound, len, "Length")),
                constraints.regex.map(pattern => if pattern.r.matches(text) then ValidationResult.valid else ValidationResult.reportError(s"regex ($pattern) not matching")),
                constraints.format.map(format => if formatToRegex(format).matches(text) then ValidationResult.valid else ValidationResult.reportError(s"format ($format) not matching"))
            ).flatten
            ValidationResult.summarize(results)

    object NumericValidation:
        def validateBound(bound: BoundConstraint[BigDecimal], value: BigDecimal): ValidationResult =
            bound.op match
        ...
        def validateConstraints(constraints: NumericConstraint.Constraints, value: BigDecimal): ValidationResult =
            val results = Seq(
                constraints.value.flatMap(_.min).map(bound => validateBound(bound, value)),
                constraints.value.flatMap(_.max).map(bound => validateBound(bound, value)),
                if constraints.integer && !value.isWhole then Some(ValidationResult.reportError(s"integer constraint not met: $value is not an integer")) else None
            ).flatten
            ValidationResult.summarize(results)
    ...
    object BinaryValidation:
        ...
        def validateBound(bound: BoundConstraint[Int], byteSize: Int): ValidationResult =
            bound.op match
                case BoundOp.Exact        => if byteSize == bound.value then ValidationResult.valid else ValidationResult.reportError(s"binary exact size constraint (${ bound.value } bytes) not met: $byteSize")
                case BoundOp.MinInclusive => if byteSize >= bound.value then ValidationResult.valid else ValidationResult.reportError(s"binary minimum size constraint (${ bound.value } bytes) not met: $byteSize")
                case BoundOp.MinExclusive => if byteSize > bound.value then ValidationResult.valid else ValidationResult.reportError(s"binary minimum size constraint (> ${ bound.value } bytes) not met: $byteSize")
                case BoundOp.MaxInclusive => if byteSize <= bound.value then ValidationResult.valid else ValidationResult.reportError(s"binary maximum size constraint (${ bound.value } bytes) not met: $byteSize")
                case BoundOp.MaxExclusive => if byteSize < bound.value then ValidationResult.valid else ValidationResult.reportError(s"binary maximum size constraint (< ${ bound.value } bytes) not met: $byteSize")

        def validateConstraints(constraints: BinaryConstraint.Constraints, data: Chunk[Byte]): ValidationResult =
            val byteSize = data.length
            val results  = Seq(
                constraints.size.flatMap(_.min).map(bound => validateBound(bound, byteSize)),
                constraints.size.flatMap(_.max).map(bound => validateBound(bound, byteSize))
            ).flatten
            ValidationResult.summarize(results)
    ...
    object ListValidation:
        def validateBound(bound: BoundConstraint[Int], size: Int): ValidationResult =
            bound.op match
        ...
        def validateConstraints(constraints: ListConstraint.Constraints, list: Chunk[Value], itemSchema: Schema): ValidationResult =
            val size          = list.size
            val sizeResults   = Seq(
                constraints.size.flatMap(_.min).map(bound => validateBound(bound, size)),
                constraints.size.flatMap(_.max).map(bound => validateBound(bound, size))
            ).flatten
            // Validate each uniqueness constraint independently
            val uniqueResults = constraints.unique.map:
                case ListConstraint.Uniqueness.Simple           => validateUniqueness(list, itemSchema)
                case ListConstraint.Uniqueness.ByFields(fields) => validateUniquenessBy(list, fields, itemSchema)
            ValidationResult.summarize(sizeResults ++ uniqueResults)
    ...
    def hasDefault(schema: Schema): Boolean = schema match
    ...
    def validateValue(schema: Schema, value: Value): ValidationResult = schema match
        ...
        case TextValue(constraints, _)      =>
            value match
                case Value.TextValue(text) => TextValidation.validateConstraints(constraints, text)
        ...
        case NumericValue(constraints, _)   =>
            value match
                case Value.NumberValue(numValue) => NumericValidation.validateConstraints(constraints, numValue)
                case Value.TextValue(textValue)  =>
                    Try(BigDecimal(textValue)) match
                        case Success(numValue) => NumericValidation.validateConstraints(constraints, numValue)
            ...
            value match
                case Value.ListOfValues(values) =>
                    ValidationResult.summarize(
                        Seq(ListValidation.validateConstraints(constraints, values, s)) ++
