...
object Validator:
    ...
    object TextValidation:
        ...
        def validateConstraints(constraint: TextConstraint.Constraint, text: String): ValidationResult = constraint match
    ...
    object NumericValidation:
        def validateConstraints(constraint: NumericConstraint.Constraint, value: BigDecimal): ValidationResult = constraint match
    ...
    def validateValue(schema: Schema, value: Value): ValidationResult = schema match
        ...
        case TextValue(constraints, _)       =>
            value match
                case Value.TextValue(text) => ValidationResult.summarize(constraints.map(c => TextValidation.validateConstraints(c, text)))
        ...
        case NumericValue(constraints, _)    =>
            value match
                case Value.NumberValue(numValue) => ValidationResult.summarize(constraints.map(c => NumericValidation.validateConstraints(c, numValue)))
                case Value.TextValue(textValue)  =>
                    Try(BigDecimal(textValue)) match
                        case Success(numValue) => ValidationResult.summarize(constraints.map(c => NumericValidation.validateConstraints(c, numValue)))
            ...
            value match
                case Value.ListOfValues(values) =>
                    ValidationResult.summarize(
                        constraints.map(c => ListValidation.validateConstraints(c, values, s)) ++
