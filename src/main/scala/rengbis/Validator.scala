...
object Validator:
    ...
    object ValidationResult:
        ...
        def summarize(values: Iterable[ValidationResult]): ValidationResult =
            val errors: Iterable[ValidationError] = values.flatMap(v => v.value match
            ...
            if (errors.isEmpty)
                ...
                ValidationResult(Chunk.fromIterable(errors))
    ...
    def validateValue(schema: Schema, value: Value): ValidationResult = schema match
        ...
        case TextValue(constraints*) => value match
        ...
        case GivenTextValue(givenValue) => value match
            ...
            case _                              =>  ValidationResult.reportError(s"expected text value; ${value.valueTypeDescription} found [value: ${value}]")
        case NumericValue() => value match
            case Value.NumberValue(value)       =>  ValidationResult.valid
            case Value.TextValue(value)         =>  Try(value.toInt) match
        ...
        case EnumValues(values*) => value match
            ...
            case _                              =>  ValidationResult.reportError(s"expected text value; ${value.valueTypeDescription} found [value: ${value}]")
        case ListOfValues(s, constraints*) => value match
            case Value.ArrayOfValues(values)    =>  ValidationResult.summarize(
        ...
        case TupleValue(options*) => value match
        ...
        case AlternativeValues(options*) =>
            options.map(s => validateValue(s, value))
        ...
        case ObjectValue(obj) => value match
            case Value.ObjectWithValues(values) =>  ValidationResult.summarize(
                obj.map((l, s) => values.get(l.label).map(v => validateValue(s, v))
                    .getOrElse(l match
